  ...
  return (
    <div
      className={`
        ...
        ${isFirst
          ? 'border-teal-400/60 bg-slate-900/60 neon-pulse'
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-3">
        <div className="relative flex-shrink-0">
          ...
          {hasStreak && (
            <span className="absolute -bottom-1 -right-1 text-lg animate-fire-flicker">🔥</span>
          )}
          {hasTrophy && (
            <span className="absolute -bottom-1 -right-1 text-lg animate-trophy-bounce">🏆</span>
          )}
        </div>
      ...
      {seller.badges && seller.badges.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-2 pt-2 border-t border-slate-700/30">
          {seller.badges.map((badge, i) => (
            <span
              key={i}
              className="px-2 py-0.5 text-[10px] rounded-full bg-yellow-500/10 text-yellow-400 border border-yellow-500/30 font-medium"
            >
              {badge}
            </span>
          ))}
      ...
      {seller.manualTags && seller.manualTags.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-1">
          {seller.manualTags.map((tag, i) => (
            <span
              key={i}
              className="px-2 py-0.5 text-[10px] rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 font-medium"
            >
              {tag}
            </span>
          ))}
